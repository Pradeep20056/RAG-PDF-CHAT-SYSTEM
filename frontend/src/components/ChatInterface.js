...
const ChatInterface = () => {
  ...
  const toggleSpeechMode = async () => {
    ...
    try {
      if (speechMode) {
        // Stop speech mode
        const response = await axios.post(`${API_BASE_URL}/speech-mode/stop`);
        if (response.data.status === 'stopped' || response.data.status === 'not_running') {
          setSpeechMode(false);
        }
      } else {
        // Start speech mode
        const response = await axios.post(`${API_BASE_URL}/speech-mode/start`);
      ...
      <div className="p-4 border-b border-gray-200 bg-white">
        <div className="flex justify-center">
          <button
            ...
            className={`flex items-center space-x-2 px-4 py-2 rounded-full font-medium transition-all ${speechMode
